...
class TestLineManagerRaisingQuery
  ...
  def execute
    puts 'Running - test_line_manager_raising_query_on_sickness.rb'
    test_01_employee_creates_sickness
    test_02_lm_raises_query
    test_03_lm_delete_sickness
    puts 'Complete - test_line_manager_raising_query_on_sickness.rb'
  end
  ...
  def test_02_lm_raises_query
    ...
    sleep 1
    SicknessExtension.new(driver).manager_raising_query(sickness_id)
    sleep 1
    SicknessExtension.new(driver).click_sickness_breadcrumb
    sleep 1
    if SicknessExtension.new(driver).sickness_status == 'query raised'
      puts 'Pass - status is Open'
    else
      puts 'FAIL - status is incorrect'
    end
...
TestLineManagerRaisingQuery.new.execute
